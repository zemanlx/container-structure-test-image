...
```bash
docker pull zemanlx/container-structure-test:v1.7.0-alpine
```
...
docker run -i --rm \
  ...
  -v ${PWD}:/test zemanlx/container-structure-test:v1.7.0-alpine \
    test \
    --image zemanlx/container-structure-test:v1.7.0-alpine \
...
After a successful build, you should have `container-structure-test:v1.7.0-alpine` image.
